...
def update(actor, critic, actor_optimizer, critic_optimizer,
            trajectories, state_size, action_size):
    trajectories = np.array(trajectories) #(s, a, r, mask)
    ...
    old_policy = actor.get_log_prob(actions, mu, std)

    criterion = torch.nn.MSELoss()

    n = len(states) # total number of samples
    arr = np.arange(n)
    ...
    for _ in range(args.model_update_num):
        ...
        for i in range(n//args.batch_size):
            ...
            actor_loss, ratio, advantages_samples = batch_surrogate_loss(actor, values_samples, targets_samples,
            ...
            # update actor & critic
            loss = actor_loss + 0.5 * critic_loss # min[-actor_obj + VFloss - entropy]
            # std is constant so entropy does not affect on optimizer
