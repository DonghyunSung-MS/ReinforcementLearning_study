...
def cost_to_go(rewards, masks, gamma):
...
def surrogate_loss(actor, values, targets, obs_samples, old_policy, act_samples):
    mu, std = actor(torch.from_numpy(obs_samples).float())
    new_policy = actor.get_log_prob(act_samples, mu, std)
